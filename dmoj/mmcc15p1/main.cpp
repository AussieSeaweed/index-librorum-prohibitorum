...
struct Update {
    int par, node;
    Update(int par, int node) : par{ par }, node{ node } {}
};

int uf[500001], sz[500001];
stack<Update> updates;
stack<int> states;

void Init(int num_nodes) {
    for (int i = 1; i <= num_nodes; i++) uf[i] = i, sz[i] = 1;
}
...
void undo() {
    ...
    if (~update.par) {
        sz[update.par] -= sz[update.node];
        uf[update.node] = update.node;
    }
}
