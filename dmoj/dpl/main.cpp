...
int N;
bool visited[3000][3000][2];
long long dp[3000][3000][2], a[3000];

long long solve(int i, int j, bool t) {
    if (visited[i][j][t]) return dp[i][j][t];
    visited[i][j][t] = true;
    if (i > j) return dp[i][j][t] = 0;

    if (t == 0) { // maximize
        long long q1 = solve(i + 1, j, !t), q2 = solve(i, j - 1, !t);
        q1 += a[i], q2 += a[j];
        dp[i][j][t] = max(q1, q2);
    } else { // minimize
        long long q1 = solve(i + 1, j, !t), q2 = solve(i, j - 1, !t);
