...
struct Node {
    ...
    Node(char c, int i, Node *parent) : c{ c }, i{ i } {
        fill(parents, parents + 20, nullptr);

        parents[0] = parent;

        for (int j = 1; j < 20 && parents[j - 1]; j++)
            parents[j] = parents[j - 1]->parents[j - 1];
    }
};
...
void Init() {
    states.reserve(1000001);
    states.push_back(new Node{ 0, -1, nullptr });
}

void TypeLetter(char L) {
    Node * &node = states.back()->children[L];

    if (!node)
        node = new Node{ L, states.back()->i + 1, states.back() };
...
char GetLetter(int P) {
    Node *node = states.back();

    for (int i = 19; i >= 0; i--)
        if (node->parents[i] && node->parents[i]->i >= P)
            node = node->parents[i];
