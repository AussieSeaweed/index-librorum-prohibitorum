...
int lend[62][300000];
bool possible[62][300000];
int N, ans;

int main() {
    scan(N);
    for (int i = 0; i < N; i++) { int x; scan(x); possible[x][i] = true; lend[x][i] = i; ans = max(ans, x); }
    for (int i = 1; i <= 60; i++) {
        for (int j = 1; j < N; j++) {
            if (possible[i][j] && possible[i][lend[i][j] - 1]) {
                possible[i + 1][j] = true;
                lend[i + 1][j] = lend[i][lend[i][j] - 1];
                ans = max(ans, i + 1);
            }
        }
    }
