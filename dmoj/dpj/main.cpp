...
double solve(int c1, int c2, int c3) {
    ...
    int c0 = N - c1 - c2 - c3;
    if (c0 == N) return dp[c1][c2][c3] = 0;
    dp[c1][c2][c3] = 1 + c1 / ((double) N) * solve(c1 - 1, c2, c3)
                        + c2 / ((double) N) * solve(c1 + 1, c2 - 1, c3)
                        + c3 / ((double) N) * solve(c1, c2 + 1, c3 - 1);
    return dp[c1][c2][c3] /= 1. - (c0 / ((double) N));
}

int main() {
    scan(N);
    for (int i = 0; i < N; i++) { int x; scan(x); cnt[x]++; }
