...
bool is_valid() {
    ...
    for (int i = 1; i <= N && valid; i++) {
        for (int j = 1; j <= N && valid; j++) {
            for (int k = 1; k <= N && valid; k++) {
                valid = valid && grid[grid[i][j]][k] == grid[i][grid[j][k]];
            }
        }
    }
    ...
    for (int i = 1; i <= N && !has_identity; i++) {
        bool cur = true;

        for (int j = 1; j <= N && cur; j++) {
            cur = cur && grid[j][i] == grid[i][j] && grid[i][j] == j;
        }
        ...
        for (int j = 1; j <= N && cur; j++) {
            ...
            for (int k = 1; k <= N && !found; k++) {
                found = grid[j][k] == grid[k][j] && grid[j][k] == i;
            }
