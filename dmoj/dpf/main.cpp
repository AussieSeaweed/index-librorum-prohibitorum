...
int lcs(int i, int j) {
    ...
    return dp[i][j] = s[i] == t[j] ? 1 + lcs(i + 1, j + 1) : max(lcs(i + 1, j), lcs(i, j + 1));
}
...
int main() {
    ...
    while (cur.first != s.size() && cur.second != t.size()) {
        if (s[cur.first] == t[cur.second]) {
            printf("%c", s[cur.first]);
            cur.first++, cur.second++;
        } else {
            if (lcs(cur.first + 1, cur.second) > lcs(cur.first, cur.second + 1))
                cur.first++;
            else
                cur.second++;
        }
