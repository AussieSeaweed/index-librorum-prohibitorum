...
unsigned long long maxn = 10, power[10], seed = 29;
unordered_map<unsigned long long, long long> cnt;
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < s[i].size(); j++) {
            ...
            for (int k = j; k < s[i].size(); k++) {
                hash += power[k - j] * (s[i][k] - 'a' + 1);
                if (!visited.count(hash)) ans += cnt[hash];
                visited.insert(hash);
            }
        }
