...
double dp[21], p[21], np[21];
long long comb[21][21], T;

int main() {
    for (int i = 0; i <= 20; i++) comb[i][i] = comb[i][0] = 1;
    for (int i = 1; i <= 20; i++) for (int j = 1; j < i; j++) comb[i][j] = comb[i - 1][j - 1] + comb[i - 1][j];
    p[0] = np[0] = 1;
    scan(T);
    for (int t = 1; t <= T; t++) {
        scanf("%lf", p + 1);
        np[1] = 1 - p[1];
        for (int i = 2; i <= 20; i++) p[i] = p[1] * p[i - 1], np[i] = np[1] * np[i - 1];
        for (int i = 1; i <= 20; i++) {
            dp[i] = 1;
            for (int j = 1; j <= i; j++)
                dp[i] += comb[i][j] * p[j] * np[i - j] * dp[i - j];
            dp[i] /= 1 - np[i];
        }
        printf("Case #%d: %.6lf\n", t, dp[20]);
    }
