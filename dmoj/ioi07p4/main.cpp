...
int N, dp[2][4][4][4][4], f[100000];
char food[100000];

inline int cnt(int a, int b, int c) {
    return popcount(((1 << a) | (1 << b) | (1 << c)) >> 1);
}

inline int solve() {
    for (int i = N - 1, head = (N - 1) & 1; i >= 0; i--, head = !head)
        for (int a = 0; a < 4; a++)
            for (int b = 0; b < 4; b++)
                for (int c = 0; c < 4; c++)
                    for (int d = 0; d < 4; d++)
                        dp[head][a][b][c][d] = max(dp[!head][f[i]][a][c][d] + cnt(f[i], a, b),
                                dp[!head][a][b][f[i]][c] + cnt(f[i], c, d));
