...
int N, dp[100000][3], c[100000][3];

int solve(int i, int j) {
    if (~dp[i][j]) return dp[i][j];
    if (i == N - 1) return dp[i][j] = c[i][j];
    return dp[i][j] = c[i][j] + max(solve(i + 1, (j + 1) % 3), solve(i + 1, (j + 2) % 3));
}

int main() {
    scan(N);
    for (int i = 0; i < N; i++) { scan(c[i][0]); scan(c[i][1]); scan(c[i][2]); }
    memset(dp, -1, sizeof dp);
    printf("%d\n", max(solve(0, 0), solve(0, 1), solve(0, 2)));
    return 0;
}