...
long long solve(int node, bool paint, int parent) {
    if (~dp[node][paint]) return dp[node][paint];
    dp[node][paint] = 1;

    for (int adj : graph[node]) {
        if (adj == parent) continue;
        dp[node][paint] = dp[node][paint] * (paint ? solve(adj, !paint, node) : solve(adj, !paint, node) + solve(adj, paint, node)) % MOD;
    }
...
int main() {
    ...
    printf("%lld\n", (solve(1, false, 0) + solve(1, true, 0)) % MOD);
    return 0;
}