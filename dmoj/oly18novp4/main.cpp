...
int N, M, a[100001], seg_tree[200000];

void setup() {
    for (int i = 0; i < N; i++) seg_tree[i + N] = i;
    for (int i = N - 1; i >= 1; i--) seg_tree[i] = a[seg_tree[i * 2]] > a[seg_tree[i * 2 + 1]] ? seg_tree[i * 2] : seg_tree[i * 2 + 1];
}
...
int query(int l, int r) {
    ...
    for (l += N, r += N; l <= r; l = (l + 1) >> 1, r = (r - 1) >> 1) {
        if (l & 1)
            ret = a[ret] > a[seg_tree[l]] ? ret : seg_tree[l];
        if (!(r & 1))
            ret = a[ret] > a[seg_tree[r]] ? ret : seg_tree[r];
    }
    return ret;
}
