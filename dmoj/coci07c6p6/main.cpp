...
#define SINF 0x3f3f3f3f3f3f3fll
using namespace std;

int N;
long long a[100001], b[100001], dp[100001];

long long get_min_cost(int i, int j) {
    long long ret = INF;
    do {
        long long cur = 0;
        for (int k = i; k <= j; k++) {
            cur += a[k] != b[k] ? abs(b[k] - a[k]) : SINF;
        }
        ret = min(ret, cur);
...
int main() {
    scan(N);
    for (int i = 1; i <= N; i++) { scan(a[i]); scan(b[i]); }
    sort(a + 1, a + N + 1);
    sort(b + 1, b + N + 1);
    for (int i = 1; i <= N; i++) {
        dp[i] = INF;
        int lim = max(1, i - 2);
        for (int j = i; j >= lim; j--) {
            dp[i] = min(dp[i], dp[j - 1] + get_min_cost(j, i));
        }
    }
    printf("%lld\n", dp[N]);
    return 0;
