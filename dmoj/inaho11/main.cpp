...
int N, M;
long long max_val[10], min_val[10], val[10], ans = 0;

long long get_product(int bitmask) {
    long long ret = 1;
...
int main() {
    ...
    fill(max_val, max_val + N, -INF);
    fill(min_val, min_val + N, INF);
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            ...
            min_val[j] = min(min_val[j], v);
        }
    }

    for (int j = 0; j < N; j++) val[j] = (max_val[j] - min_val[j]) % MOD;

    for (int i = 0; i < (1 << N); i++)
        if (popcount(i) == N - 1)
            ans = (ans + get_product(i)) % MOD;
